...
\NEW{We asked the user for the full linear density and velocity power, in
...
\NEW{Note that this discussion is only relevant because we want to be able
...
\NEW{As discussed below, we *might* also add $H(z_\star)/Hi^0(z_\star)$,
...
\AFR{Add here discussion on marginalization, moving here some of the
text currently under Emulator.}
