...
class GPEmulator:
    ...
    def __init__(self,basedir=None,p1d_label=None,skewers_label=None,
        ...
        ## Find min/max bins to use in k
        arch_k_Mpc=self.archive.data[0]["k_Mpc"]
        if arch_k_Mpc[0] > 0:
            self.first_k_bin=0
        else:
            self.first_k_bin=1
        self.last_k_bin=np.max(np.where(arch_k_Mpc<self.kmax_Mpc))+1
        self.training_k_bins=arch_k_Mpc[self.first_k_bin:self.last_k_bin]
    ...
    def _training_points_k_bin(self,archive):
        ...
        P1D_k=np.empty([len(self.archive.data),len(self.training_k_bins)])
        for aa in range(len(self.archive.data)):
            P1D_k[aa]=self.archive.data[aa]['p1d_Mpc'][self.first_k_bin:self.last_k_bin]
    ...
    def emulate_p1d_Mpc(self,model,k_Mpc,return_covar=False,z=None):
        ...
        try:
            if max(k_Mpc)>max(self.training_k_bins):
                ...
                print("Warning! Asking for k bin higher than training values.")
        except:
        ...
        try:
            ...
            print('could not make an emulator prediction')
            raise ValueError

        # Use cubic interpolation to return prediction for arbitrary k bins
        if self.emu_type=="k_bin":
            interpolator=interp1d(self.training_k_bins,pred,
        ...
        if return_covar==True:
            if self.emu_type=="k_bin":
                error_interp=interp1d(self.training_k_bins,err,
                        kind="cubic",fill_value="extrapolate")
    ...
    def saveEmulator(self):
        ...
        initParams={}
        initParams["k_bin"]=self.last_k_bin
