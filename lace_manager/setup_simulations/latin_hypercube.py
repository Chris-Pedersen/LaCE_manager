"""Setup of Latin hyper-cubes, copied from Simeon's Bird lya_emulator."""

import numpy as np

def get_hypercube_samples(param_limits, nsamples, prior_points = None,
        seed=None):
    """Given limits on a set of parameters (and optionally some prior points), 
        it will generate a hypercube design."""
    ...
    if prior_points is not None:
        if len(prior_points) == 0:
            ...
            print('before prior_points',prior_points)
            prior_points = np.array([map_to_unit_cube(pp, param_limits) for pp in prior_points])
            print('after prior_points',prior_points)
    #if specified seed, setup random generator
    if seed is not None:
        np.random.seed(seed)
    #generate normalized Latin hypercube
...
def get_random_samples(param_limits, nsamples):
    """This function randomly samples the parameters within the allowed space.
        Mostly for testing purposes, as well as evaluating how much better 
        our hypercube does."""
    ndim,nlims = np.shape(param_limits)
    assert nlims == 2
...
def default_metric_func(lhs):
    """Default metric function for the maximinlhs, below. 
        This is the sum of the Euclidean distances between each point and 
        the closest other point."""
...
def maximinlhs(n, samples, prior_points = None, metric_func = None, 
        maxlhs = 10000):
    """Generate multiple latin hypercubes and pick the one that maximises 
        the metric function.
        Arguments:
            n: dimensionality of the cube to sample [0-1]^n
            samples: total number of samples.
            prior_points: List of previously evaluated points. 
                If None, totally repopulate the space.
            metric_func: Function with which to judge the 'goodness' of 
                the generated latin hypercube.
...
def remove_single_parameter(center, prior_points):
    """Remove all values within cells covered by prior samples for a 
...
def lhscentered(n, samples, prior_points = None):
    """Generate a latin hypercube design where all samples are centered on 
        their respective cells. 
        Can specify an already existing set of points using prior_points; 
        these must also be a latin hypercube on a smaller sample, 
        but need not be centered."""
    ...
    for j in range(n):
        #Remove all values in cells covered by prior samples for this parameter.
...
def map_from_unit_cube(param_vec, param_limits):
    """Map a parameter vector from the unit cube to the original dimensions 
        of the space.
        Arguments:
